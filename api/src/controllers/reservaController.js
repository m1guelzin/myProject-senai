...
module.exports = class reservaController {
  static async createReserva(req, res) {
    ...
    // Conversão do horário de início para o formato correto (YYYY-MM-DD HH:MM:SS)
    const inicio = new Date(data_hora);
    const dataFormatada = inicio.toISOString().split("T")[0]; // "2024-12-01"
    const horaFormatada = inicio.toISOString().split("T")[1].split(".")[0]; // "22:30:00"
    ...
    const [duracaoHoras, duracaoMinutos, duracaoSegundos] = duracao
      ...
      .map(Number);
    const duracaoMs =
      ((duracaoHoras * 60 + duracaoMinutos) * 60 + (duracaoSegundos || 0)) * 1000;
    ...
    const queryConflito = `
      ...
      WHERE fkid_salas = ? 
      AND (
        (? BETWEEN data_hora AND DATE_ADD(data_hora, INTERVAL duracao SECOND)) OR
        (? BETWEEN data_hora AND DATE_ADD(data_hora, INTERVAL duracao SECOND)) OR
    ...
    const valuesConflito = [
      fkid_salas,
      dataHoraFormatada,
      fim.toISOString().split("T")[1].split(".")[0],
      dataHoraFormatada,
      fim.toISOString().split("T")[1].split(".")[0],
    ];
    ...
    try {
      connect.query(queryConflito, valuesConflito, (err, resultadosConflitos) => {
        ...
        const queryInsert = `
          ...
          VALUES (?, ?, ?, ?)
        `;
        const valuesInsert = [id_usuario, fkid_salas, dataHoraFormatada, duracao];
  ...
  static async getReservas(req, res) {
    const querySelect = `
      SELECT r.id_reserva, r.fkid_usuario, r.fkid_salas, r.data_hora, r.duracao, u.nome AS usuario_nome, s.nome AS sala_nome
      FROM reservas r
    ...
    try {
      connect.query(querySelect, (err, results) => {
        ...
        return res.status(200).json({ message: "Lista de Reservas", reservas: results });
      });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: "Erro interno do servidor" });
  ...
  static async getReservasByUser(req, res) {
    ...
    const querySelect = `
      SELECT r.id_reserva, r.fkid_usuario, r.fkid_salas, r.data_hora, r.duracao, s.nome AS sala_nome
      FROM reservas r
      INNER JOIN salas s ON r.fkid_salas = s.id_salas
    ...
    try {
      connect.query(querySelect, valuesSelect, (err, results) => {
        ...
        return res.status(200).json({ message: "Reservas do usuário", reservas: results });
      });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ error: "Erro interno do servidor" });
  ...
  static async updateReserva(req, res) {
    ...
    const queryUpdate = `
      ...
      WHERE id_reserva = ?
    `;
    const valuesUpdate = [fkid_salas, data_hora, duracao, id_reserva];
